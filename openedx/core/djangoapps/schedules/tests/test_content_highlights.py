...
class TestContentHighlights(ModuleStoreTestCase):
    ...
    def test_non_existent_course(self):
        ...
        with self.assertRaises(CourseUpdateDoesNotExist):
            get_week_highlights(self.enrolled_student, nonexistent_course_key, 0)

    @override_waffle_flag(COURSE_UPDATE_WAFFLE_FLAG, True)
    def test_empty_course(self):
        with self.assertRaises(CourseUpdateDoesNotExist):
            get_week_highlights(self.enrolled_student, self.course_key, 0)
    ...
    def test_flag_enabled(self):
        ...
        self.assertEqual(get_week_highlights(self.enrolled_student, self.course_key, 0), highlights)
    ...
    def test_course_with_highlights(self):
        with self.store.bulk_operations(self.course_key):
            highlights_value = [u'A', u'B', u'á']
            self._create_chapter(highlights=highlights_value)
